...
class PartialContentException(RestException):
    """
        ...
        resource (byte serving) due to a range header sent
        by the client. The range header is used by
        tools like wget to enable resuming of interrupted
        downloads, or split a download into multiple 
...
class MultipleChoicesException(RestException):
    """
        Indicates multiple options for the resource that 
        the client may follow. It, for instance,
        could be used to present different format 
        options for video, list files with different
...
class MovedPermanentlyException(RestException):
    """
        This and all future requests should 
        be directed to the given URI.
    """
    status_code = 301
...
class FoundException(RestException):
    """
        ...
        The HTTP/1.0 specification (RFC 1945) required 
        the client to perform a temporary redirect
        (the original describing phrase was 
        "Moved Temporarily"), but popular browsers implemented
        302 with the functionality of a 303 See Other. 
        Therefore, HTTP/1.1 added status codes 303 and
        307 to distinguish between the two behaviours. 
        However, some Web applications and frameworks
...
class SeeOtherException(RestException):
    """
        ...
        another URI using a GET method. When received
        in response to a POST (or PUT/DELETE), it should 
        be assumed that the server has received the
...
class NotModifiedException(RestException):
    """
        ...
        modified since the version specified by the request
        headers If-Modified-Since or If-None-Match. This 
        means that there is no need to retransmit
...
class UseProxyException(RestException):
    """
        The requested resource is only available 
        through a proxy, whose address is provided in the
        response. Many HTTP clients (such as Mozilla[9] 
        and Internet Explorer) do not correctly handle
        responses with this status code, primarily for 
        security reasons.
    """
    status_code = 305
...
class TemporaryRedirectException(RestException):
    """
        In this case, the request should be repeated 
        with another URI; however, future requests should
        still use the original URI. In contrast to how 302 
        was historically implemented, the request
        method is not allowed to be changed when reissuing 
        the original request. For instance, a POST
...
class PermanentRedirectException(RestException):
    """
        ...
        repeated using another URI. 307 and 308 (as
        proposed) parallel the behaviours of 302 and 301, 
...
class BadRequestException(RestException):
    """
        ...
        due to something that is perceived to be a
        client error.
    """
    status_code = 400
...
class UnauthorizedException(RestException):
    """
        Similar to 403 Forbidden, but specifically for 
        use when authentication is required and has
        failed or has not yet been provided. The 
        response must include a WWW-Authenticate header field
        containing a challenge applicable to the requested 
        resource. See Basic access authentication
...
class PaymentRequiredException(RestException):
    """
        Reserved for future use. The original intention 
        was that this code might be used as part of
        some form of digital cash or micropayment scheme, 
        but that has not happened, and this code
        is not usually used. YouTube uses this status 
        if a particular IP address has made excessive
...
class ForbiddenException(RestException):
    """
        ...
        is refusing to respond to it. Unlike a 401
        Unauthorized response, authenticating will 
        make no difference.
    """
    status_code = 403
...
class MethodNotAllowedException(RestException):
    """
        ...
        method not supported by that resource; for
        example, using GET on a form which requires 
        data to be presented via POST, or using PUT on
...
class NotAcceptableException(RestException):
    """
        ...
        generating content not acceptable according to the
...
class RequestTimeoutException(RestException):
    """
        ...
        "The client did not produce a request within 
...
class ConflictException(RestException):
    """
        ...
        such as an edit conflict in the case 
        of multiple updates.
    """
    status_code = 409
...
class GoneException(RestException):
    """
        ...
        intentionally removed and the resource should be
        purged. Upon receiving a 410 status code, the 
        client should not request the resource again in
        the future. Clients such as search engines 
        should remove the resource from their indices.
        Most use cases do not require clients and 
        search engines to purge the resource, and a
...
class LengthRequiredException(RestException):
    """
        The request did not specify the length of its 
...
class RequestedRangeNotSatisfiableException(RestException):
    """
        ...
        (byte serving), but the server cannot supply that
        portion. For example, if the client asked for a 
        part of the file that lies beyond the end of the
        file.
    """
    status_code = 416
...
class AprilFoolsException(RestException):
    """
        ...
        traditional IETF April Fools' jokes, in RFC 2324, Hyper
        Text Coffee Pot Control Protocol, and is not expected 
...
class AuthenticationTimeoutException(RestException):
    """
        Not a part of the HTTP standard, 419 Authentication 
        Timeout denotes that previously valid
        authentication has expired. It is used as an 
        alternative to 401 Unauthorized in order to
        differentiate from otherwise authenticated 
...
class EnhanceYourCalmException(RestException):
    """
        ...
        when the client is being rate limited.[16] 
        Other services may wish to implement the 429
...
class PreconditionRequiredException(RestException):
    """
        The origin server requires the request to 
        be conditional. Intended to prevent "the
        'lost update' problem, where a client GETs 
        a resource's state, modifies it, and PUTs it back
        to the server, when meanwhile a third party 
...
class TooManyRequestsException(RestException):
    """
        ...
        given amount of time. Intended for use with rate
        limiting schemes.
    """
    status_code = 429
...
class NoResponseException(RestException):
    """
        Used in Nginx logs to indicate that the server 
        has returned no information to the client and
...
class RetryWithMicrosoftException(RestException):
    """
        A Microsoft extension. The request should be 
        retried after performing the appropriate action.
    """
    status_code = 449
...
class RedirectException(RestException):
    """
        Used in Exchange ActiveSync if there either 
        is a more efficient server to use or the server
...
class RequestHeaderTooLargeException(RestException):
    """
        Nginx internal code similar to 431 but it 
        was introduced earlier in version 0.9.4
    """
    status_code = 494
...
class CertErrorException(RestException):
    """
        Nginx internal code used when SSL client 
        certificate error occurred to distinguish it from
...
class NoCertException(RestException):
    """
        Nginx internal code used when client didn't 
        provide certificate to distinguish it from 4XX
...
class HTTPtoHTTPSException(RestException):
    """
        Nginx internal code used for the plain HTTP 
        requests that are sent to HTTPS port to distinguish
...
class TokenExpiredException(RestException):
    """
        Returned by ArcGIS for Server. A code of 498 
...
class TokenrequiredException(RestException):
    """
        Returned by ArcGIS for Server. A code of 499 
...
class InternalServerErrorException(RestException):
    """
        ...
        condition was encountered and no more
...
class BandwidthLimitExceededException(RestException):
    """
        This status code is not specified 
        in any RFCs. Its use is unknown.
    """
    status_code = 509
...
class NetworkReadTimeOutException(RestException):
    """
        ...
        but is used by Microsoft HTTP proxies to
        signal a network read timeout behind the proxy 
...
class NetworkConnectTimeOutException(RestException):
    """
        This status code is not specified in any RFCs, 
        but is used by Microsoft HTTP proxies to
        signal a network connect timeout behind the 
...
class ApiError(Exception):
    """ This exception is thrown during an API call
        and therefore has the added attributes:
        :param message:             str message of 
                                    addition information from endpoint binding
        :param original_error:      Exception this 
        ...
        :param status_description:  str description of the http status
                                    or description from eagle eye bindings
    """

    def __init__(self, message=None, original_error=None,
                 http_body=None, url=None,
                 status_code=None, status_name=None,
                 status_description=None, server_details=''):
        self.message = message
        self.original_error = original_error
        self.stack = traceback.format_stack(limit=5)

        self.description = status_description is None and\
                           '' or str(status_description)
    ...
    def __repr__(self):
        return 'API Exception ( %s )' % self.status_name + \
               (self.server_details and (' - ' + self.server_details) or '')

    def __str__(self):
        values = [str_name_value(k, getattr(self, k, '')) for k in
                  ['type', 'message', 'status_name',
                   'status_code', 'url', 'server_detail']
                  # , 'description', 'stack'
                  if getattr(self, k, None)]
        return '\n' + '\n'.join(values)


HTTP_STATUS_CODES = {exception.status_code:
                         exception for exception in globals().values()
                     if getattr(exception, 'status_code', None) and
                     issubclass(exception, RestException)}
